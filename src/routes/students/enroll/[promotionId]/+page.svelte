	...
	<Card class="w-[450px]">
		...
		<CardContent>
			...
			{#if vm.authStep === 'student not found'}
				<p>Student not found in this promotion</p>
				<p>Call instructor for being added manually</p>
			{/if}
		...
		<CardFooter class="flex justify-end">
			{#if vm.authStep === 'enter email'}
				<Button onclick={vm.onJoinPromotion}>Join Promotion</Button>
			{/if}
			{#if vm.authStep === 'student found'}
				<Button onclick={vm.onVerifyOtp} disabled={vm.otp.length < 6}>Verify</Button>
			{/if}
		</CardFooter>
	</Card>
</div>
